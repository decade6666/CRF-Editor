...
- **Simple / Complete Edit Modes**: Hide advanced identifiers such as OIDs and variable names by default, and expose them consistently in complete edit mode; OIDs (form / field / codelist / option codes) are restricted at edit time to letters, digits, `-`, `_`, `.` only — invalid characters are blocked immediately without rewriting existing data
