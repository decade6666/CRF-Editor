...
- **导出能力**：支持 Word 导出与数据库导出；Word 导出内置短时频率限制，避免重复触发，并提供预览 / 导出严格表格字段一致性校验脚本
...
- **目录**：预渲染目录条目，打开即可查看与点击跳转；服务器装有 LibreOffice 时导出即带真实页码，否则在 Word 更新域后刷新页码
...
- `frontend/tests/` 当前包含 25 个 `node:test` 源码级回归文件，并引入自研轻量属性测试工具（`testProperty.js`）
