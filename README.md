...
当前仓库中：
- `backend/tests/` 当前包含 46 个 Python 测试文件（44 个 `test_*.py` 模块 + `conftest.py` + `helpers.py`），并包含部分 `hypothesis` 属性测试
- `frontend/tests/` 当前包含 44 个前端测试文件（43 个 `.test.js` + `testProperty.js`），覆盖设计器 / 访视预览 aCRF 标注几何、持久化与拖动接线等源码级契约
