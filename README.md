...
- **实时预览与快编**：设计器底部提供实时预览，支持双击预览字段快速编辑标签、颜色、横向显示与默认值等实例属性；完全模式下可在 eCRF / aCRF 视图间切换，预览字段 OID 与表单 domain 标注
...
- **导出能力**：支持 Word 导出（eCRF / aCRF）与数据库导出；Word 导出内置短时频率限制，填写线和选项尾部下划线会按列宽自适应，目录可预渲染并在可用 LibreOffice 时写入真实页码；aCRF 会在不扰动 eCRF 表格文本的前提下叠加浮动 OID / domain 注释框，同时提供预览 / 导出严格表格字段一致性校验脚本
- **项目复制与 Logo**：支持项目深拷贝与运行时 Logo 上传、复制、删除联动处理
- **访视表单预览**：在访视管理面板中直接预览各表单的字段内容布局，并复用 Word 预览行高拖拽体验
...
6. **关联表单**：将表单关联到相应的访视节点，并在访视页预览布局
...
- `frontend/tests/` 当前包含 29 个 `node:test` 源码级回归文件，并引入自研轻量属性测试工具（`testProperty.js`）
