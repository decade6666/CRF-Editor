...
2. 横向/纵向类型：由解析器依据 Word 单元格中选项之间是否有换行符判定，已较可靠。
   复核时若无明确矛盾，保持解析器给出的横/纵向，不要仅凭选项数量或长度翻转。
   仅当标签/上下文出现明确"纵向/竖向/竖排/每行一个/纵排"且与解析器类型矛盾时才更正。
3. 日期：date_format 明确为日期，或标签明显为日期含义（如"入院日期/出生日期/随机日期/访视日期"）且规则类型非日期。
   - 标签含"日期"但 date_format 空缺时为日期；含"时间/时点"到时分时为时间；日期+时间同格时为日期时间。
4. 时间：date_format 明确为时间，或标签明显为时间含义（如"采样时间/开始时间/结束时间/用餐时点/给药时间"）且规则类型非时间。
5. 数值：integer_digits 或 decimal_digits 存在时，必须为数值。
   - decimal_digits > 0 则为数值（小数）；否则为数值（整数）。
   - 标签含"剂量/体温/年龄/身高/体重/量/次数/例数/天数/周数"且有整数位/小数位时可确认为数值。
6. 标签：不需要录入、仅为说明性文本的字段（无 options、无 digits、无 date_format 且标签语义明显为静态说明或标题，如"填表说明/注意事项/小结"）。若规则引擎已判为文本且无其他特征，不要轻易改为标签。
   - 系统生成/编号类标签（如"随机号/药物编号/受试者编号/筛选号"）通常为文本型，不要改判标签。
7. 文本：无法归类时，默认文本。

常见错误模式提醒：
- 有 options 却判为文本/标签/数值：错误，必须是单选或多选。
- 单选 vs 多选混淆：看到"多选/可多选/复选/多个"必须为多选。
- 是否/有无 类标签（如"是否回收药物/有无吸烟史/是否有饮酒史"）且 options 为二值时必为单选，不可判为多选或标签。
- 数值被判为文本：出现 integer_digits/decimal_digits 必为数值。
- 日期/时间被判为文本：有 date_format 或标签含"日期/时间"语义必须改判。
- 纵/横向类型不要仅凭选项数量或长度翻转解析器判定；仅在有明确标签矛盾时才更正。
...
表单名称：不良事件
字段列表：
  0. 标签="对不良事件采取的措施"，类型="多选（纵向）"，选项=['未采取措施','药物治疗','非药物治疗','其他，请描述']
  1. 标签="学历"，类型="单选"，选项=['高中及高中以下','本科','硕士','博士']
  2. 标签="是否回收药物"，类型="文本"，选项=['是','否']

示例输出：
[
  {"index": 0, "ok": true},
  {"index": 1, "ok": true},
  {"index": 2, "ok": false, "suggested_type": "单选", "reason": "存在互斥二值选项，是否类标签应为单选"}
]
"""
...
def _build_user_prompt(form_name: str, fields: List[dict]) -> str:
    ...
    for i, f in enumerate(_get_real_fields(fields)):
        ...
        if f.get("options"):
            ...
            for o in f["options"]:
                if isinstance(o, str):
                    opt_texts.append(o.strip())
                elif isinstance(o, dict):
                    d = str(o.get("decode", "") or "").strip()
                    if d:
                        opt_texts.append(d)
            if opt_texts:
                extra = f"，选项={opt_texts}"
...
def _get_ai_review_config():
    return get_config().ai_config


def _is_ai_review_enabled() -> bool:
    cfg = _get_ai_review_config()
    return bool(cfg.enabled and cfg.api_url and cfg.api_key and cfg.model)
...
async def _run_ai_review_task(temp_id: str, task: AIReviewTask, forms: List[dict]) -> None:
    ...
    if task.total == 0:
        ...
        return
    try:
        sem = asyncio.Semaphore(_resolve_max_concurrency(cfg))
        api_format = cfg.api_format or "openai"
        async with httpx.AsyncClient(timeout=cfg.timeout, follow_redirects=True) as client:
            ...
            for review_task in asyncio.as_completed(review_tasks):
                form_index, diffs = await review_task
                if diffs:
                    task.suggestions[form_index] = diffs
                task.completed += 1
        task.status = "done"
    except Exception:
        task.status = "failed"
        task.error = "AI复核任务失败"
        logger.exception("AI复核后台任务失败 temp_id=%s", temp_id)


def get_ai_task(temp_id: str) -> Optional[AIReviewTask]:
    return _ai_tasks.get(temp_id)


def remove_ai_task(temp_id: str) -> None:
    _ai_tasks.pop(temp_id, None)
...
def cleanup_old_ai_tasks(max_age: int = 3600) -> int:
    ...
    if expired_ids:
        logger.info("已清理过期AI复核任务 count=%d", len(expired_ids))
