...
def _migrate_project_soft_delete_and_ordering(engine):
    """给 project 表添加 order_index 和 deleted_at 列，并给 form_field 添加 order_index 列（重命名 order_index）"""
    insp = inspect(engine)
    with engine.begin() as conn:
        # 1. Project 迁移
        ...
        # 2. FormField 迁移 (order_index -> order_index)
        if insp.has_table("form_field"):
            cols = [c["name"] for c in insp.get_columns("form_field")]
            if "order_index" not in cols:
                if "order_index" in cols:
                    # SQLite 不支持 RENAME COLUMN，用 ALTER ADD + UPDATE
                    conn.execute(text('ALTER TABLE form_field ADD COLUMN order_index INTEGER DEFAULT 1 NOT NULL'))
                    conn.execute(text('UPDATE form_field SET order_index = order_index + 1'))
                else:
                    conn.execute(text('ALTER TABLE form_field ADD COLUMN order_index INTEGER DEFAULT 1 NOT NULL'))

        # 3. Project order_index 数据回填（按 owner_id 分组）
        if insp.has_table("project"):
