...
def copy_visit(visit_id: int, session: Session = Depends(get_session)):
...
def add_visit_form(visit_id: int, form_id: int, session: Session = Depends(get_session)):
    """将表单加入访视"""
...
def remove_visit_form(visit_id: int, form_id: int, session: Session = Depends(get_session)):
    """从访视移除表单"""
    from sqlalchemy import select
    from src.models.visit_form import VisitForm
    vf = session.scalar(select(VisitForm).where(VisitForm.visit_id == visit_id, VisitForm.form_id == form_id))
