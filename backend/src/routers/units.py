...
from src.dependencies import get_current_user, verify_project_owner, verify_project_unit_owner
...
def update_unit(unit_id: int, data: UnitUpdate, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
...
def get_unit_references(unit_id: int, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
...
def delete_unit(unit_id: int, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
...
def batch_unit_references(project_id: int, data: BatchDeleteRequest, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    ...
    valid_unit_ids = set(session.scalars(
        select(Unit.id).where(Unit.project_id == project_id, Unit.id.in_(data.ids))
    ).all())
    stmt = (
