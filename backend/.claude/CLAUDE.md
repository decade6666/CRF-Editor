...
## 核心目录
- `src/routers/`（13 个 Python 文件）：认证、管理员、项目、访视、表单、字段、字典、单位、导入导出、设置接口。
- `src/services/`（13 个 Python 文件）：认证、用户管理、导入、导出、排序、项目克隆、项目导入、AI 复核、Docx 截图缓存、列宽规划等重逻辑。
- `src/models/`（11 个 Python 文件）：Project、Visit、Form、VisitForm、FieldDefinition、FormField、CodeList、CodeListOption、Unit、User 等 ORM 模型。
- `src/schemas/`（7 个 Python 文件）：项目、访视、表单、字段、字典、单位等 Pydantic 请求 / 响应结构。
- `src/repositories/`（6 个 Python 文件）：基础仓储、项目、字段定义、字段实例等数据库访问封装。
