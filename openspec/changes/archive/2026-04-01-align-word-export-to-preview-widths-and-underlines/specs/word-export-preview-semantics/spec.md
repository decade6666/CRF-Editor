...
For any horizontal table input, the final planned width total is less than or equal to the available page-width budget.

### Property: Unified slot-max monotonicity
For any unified table, if a column slot receives a greater maximum semantic demand, that slot's final width does not decrease.

### Property: Shared-scope consistency
For any single unified table, all inline blocks inside that table are governed by one shared width-planning scope.

### Property: Proportional fallback monotonicity
For any two slots A and B, if A has greater intrinsic demand than B before fallback scaling, A remains at least as wide as B after fallback scaling.
...
### Property: Multiline semantic consistency
For any multiline `default_value`, preview and export preserve the same multiline semantic category.

### Property: Planning idempotence
Running the same width-planning logic repeatedly on the same semantic input produces the same result.
